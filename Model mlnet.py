...
""").lower()
    ...
    def on_initialize(self):
        ...
        class ConvBlock(nn.ModelBase):
            def on_build(self, in_ch, out_ch):              
                self.conv = nn.Conv2D (in_ch, out_ch, kernel_size=3, strides=2, padding='SAME')
        ...
        # class MetricLearningNetwork(nn.ModelBase):
        #     def on_build(self, resolution=224, out_ch=10, base_ch=32):
        #         self.conv0 = nn.Conv2D( 3, base_ch, kernel_size=1, strides=1, padding='SAME')
                
        #         self.conv1 = ConvBlock( base_ch, base_ch*2)                
        #         #self.bp1 = nn.BlurPool (filt_size=3)
                
        #         self.conv2 = ConvBlock( base_ch*2, base_ch*4)                
        #         #self.bp2 = nn.BlurPool (filt_size=3)
                
        #         self.conv3 = ConvBlock( base_ch*4, base_ch*8)                
        #         #self.bp3 = nn.BlurPool (filt_size=3)
                
        #         self.conv4 = ConvBlock( base_ch*8, base_ch*16)                
        #         #self.bp4 = nn.BlurPool (filt_size=3)
                
        #         self.conv5 = ConvBlock( base_ch*16, base_ch*16)                
        #         #self.bp5 = nn.BlurPool (filt_size=3)

        #         self.dense1 = nn.Dense (16*base_ch, out_ch)
        ...
        class MetricLearningNetwork(nn.ModelBase):
            def on_build(self, resolution=224, out_ch=10, base_ch=32):
                self.conv0 = nn.Conv2D( 3, base_ch, kernel_size=1, strides=1, padding='SAME')
                self.conv1 = nn.Conv2D( base_ch, 2*base_ch,    kernel_size=3, strides=2, padding='SAME')
                self.conv2 = nn.Conv2D( 2*base_ch, 4*base_ch,  kernel_size=3, strides=2, padding='SAME')
                self.conv3 = nn.Conv2D( 4*base_ch, 8*base_ch,  kernel_size=3, strides=2, padding='SAME')
                self.conv4 = nn.Conv2D( 8*base_ch, 16*base_ch,  kernel_size=3, strides=2, padding='SAME')
                self.conv5 = nn.Conv2D( 16*base_ch, 16*base_ch,  kernel_size=3, strides=2, padding='SAME')
                self.dense1 = nn.Dense (16*base_ch, out_ch)

            def forward(self, inp):
                x = inp
                x = tf.nn.leaky_relu(self.conv0(x), 0.2)
                ...
                x = tf.nn.leaky_relu(self.conv5(x), 0.2)
                x = tf.reduce_max(x, axis=[-2,-1])
                x = self.dense1(x)
                return x
        ...
        class MetricLearningNetwork(nn.ModelBase):
            ...
            def on_build(self, patch_size, in_ch, base_ch = 16, use_fp16 = False):
                ...
                level_chs = { i-1:v for i,v in enumerate([ min( base_ch * (2**i), 512 ) for i in range(layers_len+1)]) }

                self.in_conv = nn.Conv2D( in_ch, level_chs[-1], kernel_size=1, padding='VALID', dtype=conv_dtype)
                
                self.denses = []
                self.sub_convs = []
                for j in range(2,layers_len):
                    
                    convs = []
                    self.sub_convs.append(convs)
                    
                    for i, (kernel_size, strides) in enumerate(layers[:j]):
                        convs.append ( nn.Conv2D( level_chs[i-1], level_chs[i], kernel_size=kernel_size, strides=strides, padding='SAME', dtype=conv_dtype) )

                    self.denses.append( nn.Dense(level_chs[j-1], max(1, 10 // layers_len) ) )

            def forward(self, x):
                x = inp = tf.nn.leaky_relu( self.in_conv(x), 0.2 )
                out = []
                for sub_conv, dense in zip(self.sub_convs, self.denses):
                    x = inp
                    for conv in sub_conv:
                        x = tf.nn.leaky_relu( conv(x), 0.2 )
                    out.append( dense ( tf.reduce_max(x, axis=[-2,-1]) )  )
                    
                return tf.concat(out, -1)                    
                
        def cosine_similarity(x1,x2, axis=1, eps=1e-6):
            w12 = tf.reduce_sum(x1*x2, axis=axis)
            w1 = tf.reduce_sum(x1*x1, axis=axis)
            w2 = tf.reduce_sum(x2*x2, axis=axis)
            n12 = tf.sqrt( tf.math.maximum(w1*w2, eps) )
            return w12 / n12
            
        def triplet_loss(anchor, positive, negative, weight=10.0):
            distance_positive = tf.reduce_sum( tf.square(anchor - positive), axis=1 )
            distance_negative = tf.reduce_sum( tf.square(anchor - negative), axis=1 )
            cos_reg = tf.reduce_sum( cosine_similarity(negative - anchor, positive - anchor), axis=0 )
            losses = tf.reduce_mean( weight*tf.math.softplus(distance_positive - distance_negative + 1 - 0.01 * cos_reg) )
            return losses
        ...
        with tf.device (models_opt_device):
            ...
            if self.is_training:
                ...
                self.mlnet = MetricLearningNetwork(patch_size=resolution // 2, in_ch=3, base_ch=64, name='mlnet')
                self.model_filename_list += [ [self.mlnet, 'mlnet.npy'] ]
                
                self.mlnet_opt = OptimizerClass(lr=lr, lr_dropout=lr_dropout, lr_cos=lr_cos, clipnorm=clipnorm, name='mlnet_opt')
                self.mlnet_opt.initialize_variables (self.mlnet.get_weights(), vars_on_cpu=optimizer_vars_on_cpu, lr_dropout_on_cpu=self.options['lr_dropout']=='cpu')
                self.model_filename_list += [ (self.mlnet_opt, 'mlnet_opt.npy') ]
        ...
        if self.is_training:
            ...
            for gpu_id in range(gpu_count):
                with tf.device( f'/{devices[gpu_id].tf_dev_type}:{gpu_id}' if len(devices) != 0 else f'/CPU:0' ):
                    ...
                    gpu_pred_src_src_ml = self.mlnet(gpu_pred_src_src_masked_opt)
                    gpu_target_src_ml = self.mlnet(gpu_target_src_masked_opt)
                    
                    gpu_pred_dst_dst_ml = self.mlnet(gpu_pred_dst_dst_masked_opt)
                    gpu_target_dst_ml = self.mlnet(gpu_target_dst_masked_opt)
                    
                    with tf.device(f'/CPU:0'):
                        gpu_pred_src_src_ml_shuffle = tf.random.shuffle(tf.range(bs_per_gpu))
                        gpu_target_src_ml_shuffle = tf.random.shuffle(tf.range(bs_per_gpu))
                        gpu_pred_dst_dst_ml_shuffle = tf.random.shuffle(tf.range(bs_per_gpu))
                        gpu_target_dst_ml_shuffle = tf.random.shuffle(tf.range(bs_per_gpu)) 
                        
                    gpu_pred_src_src_ml_shuffle = tf.gather( gpu_pred_src_src_ml, gpu_pred_src_src_ml_shuffle )
                    gpu_target_src_ml_shuffle   = tf.gather( gpu_target_src_ml, gpu_target_src_ml_shuffle )
                    gpu_pred_dst_dst_ml_shuffle = tf.gather( gpu_pred_dst_dst_ml, gpu_pred_dst_dst_ml_shuffle )
                    gpu_target_dst_ml_shuffle   = tf.gather( gpu_target_dst_ml, gpu_target_dst_ml_shuffle)
                    
                    pd_target_src   = pairwise_distances(gpu_target_src_ml, gpu_target_src_ml)
                    pd_pred_src_src = pairwise_distances(gpu_pred_src_src_ml, gpu_pred_src_src_ml)
                    
                    ml_src_loss = tf.reduce_mean( 10*tf.square(pd_pred_src_src-pd_target_src) ) + \
                                  tf.reduce_mean( 10*tf.square( tf.reduce_mean(gpu_target_src_ml, 0) - tf.reduce_mean(gpu_pred_src_src_ml, 0) ) )
                    
                    pd_target_dst   = pairwise_distances(gpu_target_dst_ml, gpu_target_dst_ml)
                    pd_pred_dst_dst = pairwise_distances(gpu_pred_dst_dst_ml, gpu_pred_dst_dst_ml)
                    
                    ml_dst_loss = tf.reduce_mean( 10*tf.square(pd_pred_dst_dst-pd_target_dst)) + \
                                  tf.reduce_mean( 10*tf.square( tf.reduce_mean(gpu_target_dst_ml, 0) - tf.reduce_mean(gpu_pred_dst_dst_ml, 0) ) )
                   
                   
                    gpu_src_loss = ml_src_loss
                    # if resolution < 256:
                    ...
                    gpu_src_loss += tf.reduce_mean ( tf.square ( gpu_target_src_masked_opt - gpu_pred_src_src_masked_opt ) )#, axis=[1,2,3])
                    # if eyes_mouth_prio:
                    ...
                    gpu_dst_loss += tf.reduce_mean ( tf.square(  gpu_target_dst_masked_opt- gpu_pred_dst_dst_masked_opt ) )#, axis=[1,2,3])
                    ...
                    gpu_MLNET_loss = triplet_loss(gpu_target_src_ml, gpu_target_src_ml_shuffle, gpu_pred_src_src_ml_shuffle ) + \
                                     triplet_loss(gpu_target_dst_ml, gpu_target_dst_ml_shuffle, gpu_pred_dst_dst_ml_shuffle )
                    
                    gpu_MLNET_loss_gvs += [ nn.gradients (gpu_MLNET_loss, self.mlnet.get_weights() ) ]
            ...
            with tf.device (models_opt_device):
                ...
                MLNET_loss_gv_op = self.mlnet_opt.get_update_op (nn.average_gv_list(gpu_MLNET_loss_gvs))
            ...
            def MLNET_train(warped_src, target_src, target_srcm, target_srcm_em,  \
                              warped_dst, target_dst, target_dstm, target_dstm_em, ):
                nn.tf_sess.run ([MLNET_loss_gv_op], feed_dict={self.warped_src :warped_src,
                                                            self.target_src :target_src,
                                                            self.target_srcm:target_srcm,
                                                            self.target_srcm_em:target_srcm_em,
                                                            self.warped_dst :warped_dst,
    ...
    def onTrainOneIter(self):
        ...
        self.MLNET_train(warped_src, target_src, target_srcm, target_srcm_em, warped_dst, target_dst, target_dstm, target_dstm_em)
